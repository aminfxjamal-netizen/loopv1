...
import { addFollowUp, getDueFollowUps } from '@/lib/followup-service';
...
export default function Workspace() {
      ...
      try {
        ...
        if (!existingUser) {
          ...
          const { data: newUser } = await supabase
            .from('users')
            .insert([{
              name: parsed.name || 'User', email: userEmail, password: parsed.password || '',
              plan: 'trial', trial_start: new Date().toISOString(),
  ...
  useEffect(() => {
    ...
    if (dueFollowUps.length > 0 && userId) {
      const followUpMessages = dueFollowUps.map(f => ({
        ...
        content: `Follow-up reminder: No reply from ${f.recipientEmail} regarding "${f.subject}". Want me to draft a follow-up?`
      }));
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (userId && !convId) {
        try { const conv = await createConversation(userId, userContent.substring(0, 60)); convId = conv.id; setActiveConversationId(convId); const convs = await getConversations(userId); setConversations(convs || []); } catch {}
      }
      ...
      if (data.isDraft && data.content) {
        const toMatch = data.content.match(/^To:\s*(.+)$/m), subjectMatch = data.content.match(/^Subject:\s*(.+)$/m);
  ...
  const loadConversation = async (convId: string) => {
    setActiveConversationId(convId); setSidebarOpen(false);
    try { const msgs = await getMessages(convId); setMessages((msgs || []).map((m: any) => ({ id: m.id, role: m.role, content: m.content, isDraft: m.is_draft, recipient: m.recipient || '', subject: m.subject || '' }))); } catch {}
  };
  ...
  const handleApproveDraft = async (message: Message) => {
    ...
    try {
      ...
      const data = await response.json();
      setMessages(prev => [...prev, { id: Math.random().toString(36).substring(7), role: 'assistant', content: data.success ? `Email sent to ${message.recipient}. I will follow up in 3 days if there is no reply.` : `Failed to send: ${data.error || 'Unknown error'}` }]);
      if (data.success && userId) {
        const followUpDate = new Date(); followUpDate.setDate(followUpDate.getDate() + 3);
        addFollowUp({ userId, recipientEmail: message.recipient, subject: message.subject, sentDate: new Date().toISOString(), followUpDate: followUpDate.toISOString(), status: 'pending' });
      }
    } catch {
      ...
      <main className="flex-1 flex flex-col h-full relative min-w-0">
        ...
        <div className={`fixed right-0 top-0 h-full w-[320px] bg-[#0d0d0d] border-l border-white/5 z-50 p-6 overflow-y-auto transform transition-transform duration-300 ease-in-out ${showConnectedApps ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="bg-white/5 rounded-xl p-4 border border-white/5 mb-4">
            <div className="flex items-center gap-2"><Mail size={18} className="text-red-400" /><span className="text-white font-medium text-sm">Gmail</span><span className={`h-2 w-2 rounded-full ml-auto ${isConfigured ? 'bg-green-500' : 'bg-gray-600'}`}></span></div>
            ...
            {showGmailForm && (
              <div className="mt-3 space-y-2"><input type="email" value={gmailEmail} onChange={(e) => setGmailEmail(e.target.value)} placeholder="Your Gmail address" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-blue-500" /><input type="password" value={gmailPassword} onChange={(e) => setGmailPassword(e.target.value)} placeholder="App Password" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-blue-500" /><button onClick={handleConnectGmail} disabled={!gmailEmail || !gmailPassword} className="w-full bg-blue-500 text-white text-sm font-medium py-2 rounded-lg hover:bg-blue-600 disabled:opacity-50">Save & Connect</button></div>
            )}
          </div>
