...
export default function LoginPage() {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Authenticate user session with Supabase auth server
      ...
      if (error) {
        ...
        setSuccessMessage('Authentication verified. Accessing your corporate space...');
        
        // Match returning user journey blueprint: Bypass onboarding and drop straight to workspace
        setTimeout(() => {
          router.push('/workspace');
        }, 1200);
      }
    } catch (err: any) {
      setErrorMessage('An unexpected authorization timeout occurred. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-[#111827] font-sans antialiased flex flex-col justify-between p-6 md:p-8">
      ...
      <header className="max-w-7xl w-full mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-5">
          <span className="text-xs font-semibold text-[#6B7280] hidden sm:inline">New to the network node?</span>
          <button 
      ...
      <main className="flex-1 flex items-center justify-center max-w-md w-full mx-auto my-12">
        <div className="w-full bg-white border border-[#E5E7EB] rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
          <div className="space-y-1.5 text-center">
            <h1 className="text-xl font-black tracking-tight text-[#111827]">Access Active Space</h1>
            <p className="text-xs text-[#6B7280] font-medium">Verify credentials to re-initialize your AI pipelines.</p>
          </div>
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] font-bold text-[#2563EB] hover:underline cursor-pointer">Recover Entry</span>
              </div>
              <input
                type="password"
                required
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-10 bg-[#2563EB] hover:bg-[#1D4ED8] disabled:bg-[#93C5FD] text-white text-xs font-bold rounded-xl shadow-sm transition flex items-center justify-center mt-4"
            >
              {loading ? (
