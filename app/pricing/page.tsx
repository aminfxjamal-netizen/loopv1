...
export default function PricingPage() {
  ...
  const handleFreeTrial = () => {
    ...
    if (userData) {
      ...
      localStorage.setItem('loop_user', JSON.stringify({ ...parsed, plan: 'trial', trialStart: new Date().toISOString(), trialEnd: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString() }));
    }
  ...
  const handlePaidPlan = (plan: string) => {
    const userData = localStorage.getItem('loop_user_data');
    if (userData) localStorage.setItem('loop_selected_plan', JSON.stringify({ ...JSON.parse(userData), plan }));
    router.push('/billing');
  };
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white font-sans">
      <nav className="flex items-center justify-between px-4 md:px-6 py-5 max-w-7xl mx-auto">
        ...
        <Link href="/signup" className="flex items-center gap-1 text-sm text-gray-400 hover:text-white transition-colors"><ArrowLeft size={14} /> Back</Link>
      </nav>
      <div className="max-w-7xl mx-auto px-4 py-16 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto mt-12">
          ...
          <div className="border border-white/5 rounded-2xl p-8 flex flex-col bg-[#0d0d0d]">
            ...
            <button onClick={handleFreeTrial} className="bg-white/10 text-white text-sm font-medium text-center py-3 rounded-xl hover:bg-white/20 transition-colors w-full">Start Free Trial</button>
          ...
          <div className="border-2 border-blue-500 rounded-2xl p-8 flex flex-col bg-[#0d0d0d] relative">
            ...
            <button onClick={() => handlePaidPlan('pro')} className="bg-blue-500 text-white text-sm font-medium text-center py-3 rounded-xl hover:bg-blue-600 transition-colors w-full flex items-center justify-center gap-2">Choose Pro <ArrowRight size={14} /></button>
          ...
          <div className="border border-white/5 rounded-2xl p-8 flex flex-col bg-[#0d0d0d]">
            ...
            <button onClick={() => handlePaidPlan('business')} className="bg-white/10 text-white text-sm font-medium text-center py-3 rounded-xl hover:bg-white/20 transition-colors w-full flex items-center justify-center gap-2">Choose Business <ArrowRight size={14} /></button>
